...
class ClickListener(val action: ClickEvent => Unit)(implicit wrapper: WrapperRegistry) extends com.vaadin.event.MouseEvents.ClickListener with Listener {
  def click(e: com.vaadin.event.MouseEvents.ClickEvent) = action(ClickEvent(wrapper.get[Table](e.getComponent).get, e.getButton, e.getClientX, e.getClientY, e.getRelativeX, e.getRelativeY, e.isDoubleClick, e.isAltKey, e.isCtrlKey, e.isMetaKey, e.isShiftKey))
}