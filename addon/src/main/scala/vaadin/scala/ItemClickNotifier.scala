...
package internal {
  class ItemClickListener(val action: ItemClickEvent => Unit) extends com.vaadin.event.ItemClickEvent.ItemClickListener with Listener {
    def itemClick(e: com.vaadin.event.ItemClickEvent) = action(ItemClickEvent(wrapperFor[Table](e.getComponent).get, e.getItem, e.getItemId, Option(e.getPropertyId), e.getButton, e.getClientX, e.getClientY, e.getRelativeX, e.getRelativeY, e.isDoubleClick, e.isAltKey, e.isCtrlKey, e.isMetaKey, e.isShiftKey))
  }
}
