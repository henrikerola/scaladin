...
trait Item extends Wrapper {
  ...
  def getPropertyOption(id: Any): Option[Property[_, _]] = optionalWrapProperty(id, p.getItemProperty(id))
  ...
  protected def optionalWrapProperty(propertyId: Any, property: com.vaadin.data.Property[_]): Option[Property[_, _]] = property match {
    case p: com.vaadin.data.Property[_] => Some(wrapProperty(propertyId, p))
    case _ => None
  }

  //override if needed
  protected def wrapProperty(propertyId: Any, unwrapped: com.vaadin.data.Property[_]): Property[_, _] = new BasicProperty(unwrapped)
}
