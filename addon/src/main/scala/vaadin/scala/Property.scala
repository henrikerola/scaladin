...
object Property {
  def apply[T](value: T): Property[T, T] = new ObjectProperty[T](value)

  def unapply(property: Property[_, _]): Option[Any] = {
    if (property != null) property.value
    else None
  }
}

trait Property[T, V] extends Wrapper {
  def p: com.vaadin.data.Property[V]
  ...
  def value_=(value: Any): Unit = {} //p.setValue(value.asInstanceOf[T]) FIXME
  def getType: Class[_ <: T] = null //p.getType FIXME
...
trait PropertyViewer extends Wrapper {
  ...
  def property: Option[Property[_, _]] = p.getPropertyDataSource() match {
    case p: com.vaadin.data.Property[_] => Some(new BasicProperty(p))
    case null => None
  }

  def property_=(property: Option[Property[_, _]]) = property match {
    case Some(prop) => p.setPropertyDataSource(prop.p)
    case None => p.setPropertyDataSource(null)
  }

  def property_=(property: Property[_, _]) = p.setPropertyDataSource(property.p)
}
...
class FunctionProperty[T](getter: Unit => T, setter: T => Unit = null)(implicit m: ClassTag[T]) extends Property[T, T] {
  //delegate
