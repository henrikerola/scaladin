...
class AbsoluteLayout(override val p: com.vaadin.ui.AbsoluteLayout with AbsoluteLayoutMixin = new com.vaadin.ui.AbsoluteLayout with AbsoluteLayoutMixin)
  ...
  class ComponentPosition(override val p: com.vaadin.ui.AbsoluteLayout#ComponentPosition) extends Wrapper {
    ...
    def top_=(top: Measure): Unit = p.setTop(top.value.floatValue, com.vaadin.server.Sizeable.Unit.values.apply(top.unit.id))
    ...
    def right_=(right: Measure): Unit = p.setRight(right.value.floatValue, com.vaadin.server.Sizeable.Unit.values.apply(right.unit.id))
    ...
    def bottom_=(bottom: Measure): Unit = p.setBottom(bottom.value.floatValue, com.vaadin.server.Sizeable.Unit.values.apply(bottom.unit.id))
    ...
    def left_=(left: Measure): Unit = p.setLeft(left.value.floatValue, com.vaadin.server.Sizeable.Unit.values.apply(left.unit.id))
  }
}